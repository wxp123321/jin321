...
Page({
  ...
  data: {
    ...
    region:[],
    region2:['北京'],
    region3: ['东城区', '西城区', '崇文区', '宣武区', '朝阳区', '丰台区', '石景山区', '海淀区', '门头沟区', '房山区', '通州区', '顺义区', '昌平区', '大兴区', '平谷区', '怀柔区', '密云县', '延庆县', '其他'],
    index:0,
    index2:0,
    index3:0
  },
  ...
  addAddress:function(){
    ...
    wx.getStorage({
      ...
      success: function (res) {
        wx.request({
          url: 'https://www.jin321.cn/jin321/wx/insertUseraddress.do',
          method: 'POST',
          data: {
            uid: res.data,
            ubname:that.data.username,
            uphonenum: that.data.phoneNumber,
            umail: that.data.postcode,
            uprovince: that.data.province,
            ucity: that.data.city,
            uarea: that.data.county,
  ...
  getProvince(e){
    ...
    var value = this.data.region[id];
    var cities = [];
    var counties = [];
    for(var key in address[value]){
      cities.push(key);
    }
    for(var key in address[value][cities[0]]){
      counties.push(address[value][cities[0]][key]);
    }
    this.setData({
      province: value
    });
  ...
  getCounty(e){
    var id = e.detail.value;
    var value = this.data.region3[id];
    this.setData({
      index3:id
    });
    this.setData({
